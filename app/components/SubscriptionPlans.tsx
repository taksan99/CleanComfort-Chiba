...
export default function SubscriptionPlans() {
  ...
  const imageSections = useMemo(() => ["subscriptionImage"], [])
  const { imageUrls } = useMultipleImageCache(imageSections)
  ...
  useEffect(() => {
    if (imageUrls.subscriptionImage === null) {
      setIsImageLoading(false)
      setImageLoadError("背景画像の読み込みに失敗しました。デフォルトの背景を使用します。")
    } else if (imageUrls.subscriptionImage) {
      setIsImageLoading(false)
      setImageLoadError(null)
    }
  }, [imageUrls.subscriptionImage])

  const backgroundImage = imageUrls.subscriptionImage || "/placeholder.svg"
  ...
  return (
    <section className="py-16 bg-cover bg-center relative" style={{ backgroundImage: `url(${backgroundImage})` }}>
      {imageLoadError && (
        <div className="absolute top-0 left-0 right-0 bg-yellow-100 text-yellow-800 px-4 py-2 text-center">
